...
const SHEET_TOOLS = [
  ...
  { name: "get_today_summary", description: "오늘 팀원별 작업량+매입 현황 집계.", input_schema: { type: "object" as const, properties: {} } },
  {
    ...
    description:
      "고물상 외부 시장 시세(시세조사 탭에 수집된 최신 스냅샷)를 조회한다. 우리 매입/판매 단가(get_prices)와 다른, 외부 시세다. 특정 품목은 web_search로 실시간 확인 가능.",
    input_schema: { type: "object" as const, properties: { item: { type: "string", description: "품목명(선택). 없으면 전체" } }, required: [] },
  },
];
...
- 외부 시장 시세는 먼저 get_market_prices(시세조사 탭 수집분)로 확인하고, 없는 품목이나 실시간 확인이 필요하면 web_search·web_fetch 로 찾아본다.
- 뉴스/시장정보 등 그 밖의 외부 최신 정보가 필요하면 web_search·web_fetch 로 직접 찾아본다.
...
async function runSheetTool(name: string, input: any): Promise<string> {
  switch (name) {
    ...
    case "get_market_prices": {
      ...
      const list = q ? market.filter((m) => m.item.includes(q) || m.category.includes(q)) : market;
